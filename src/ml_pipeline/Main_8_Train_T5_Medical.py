#!/usr/bin/env python3
"""
🏥 T5-SMALL MEDICAL DIAGNOSIS TRAINER
🎯 Train T5-Small for Disease Diagnosis Task
👥 For: General public users - Symptoms → Disease Name

Model: T5-Small (60M parameters)
Task Format: "diagnose disease: {symptoms}" → "{disease_name}"
Architecture: Encoder-Decoder (better for Q&A than Causal LM)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('logs/t5_training.log', encoding='utf-8')
    ]
)
...
class T5MedicalTrainer:
    """T5-SMALL trainer for medical diagnosis"""
    ...
    def __init__(self, 
        ...
        logger.info("="*70)
        logger.info("🏥 T5-SMALL MEDICAL DIAGNOSIS TRAINER")
        logger.info("="*70)
    ...
    def test(self, test_data):
        ...
        # Specialty-wise accuracy
        specialty_accuracy = {}
        for item, pred in zip(test_data, all_predictions):
            specialty = item.get('specialty', 'unknown')
            if specialty not in specialty_accuracy:
                specialty_accuracy[specialty] = {'correct': 0, 'total': 0}
            
            specialty_accuracy[specialty]['total'] += 1
            if pred.strip().lower() == item['disease'].strip().lower():
                specialty_accuracy[specialty]['correct'] += 1
        
        logger.info("📊 Accuracy by Specialty:")
        logger.info("-"*70)
        for specialty, stats in sorted(specialty_accuracy.items(), key=lambda x: -x[1]['total']):
            acc = stats['correct'] / stats['total']
            logger.info(f"   {specialty:20s}: {acc:.2%} ({stats['correct']}/{stats['total']})")
    ...
    def save_model(self, output_dir=None):
        ...
        if output_dir is None:
            ...
            output_dir = Path(f"models/t5_diagnosis_{timestamp}")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="T5-SMALL Medical Diagnosis Trainer")
    ...
    parser.add_argument("--model", type=str, default="t5-small", help="Model name (default: t5-small, try: google/flan-t5-small)")
